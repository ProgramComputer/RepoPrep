...
async function answerRubricQuestionsWithGemini(repoAnalysis: any, rubric: string): Promise<InterviewQuestion[]> {
  try {
    ...
    // Include code samples for important files
    ...
    const prompt = `
      ...
      In addition to the information in the analysis, the repomix tool works by:
      1. Cloning the GitHub repository to a temporary directory
      2. Analyzing the file structure and content
      3. Identifying technologies, components, and dependencies
      4. Extracting key code samples from important files
      5. Generating a comprehensive XML representation of the repository
      ...
      Your answers should:
      1. Be specific about what you can determine from the repository analysis and code samples
      2. Reference specific code samples when relevant to your answers
      3. Acknowledge when the analysis doesn't provide enough information for a complete answer
      4. Be honest about limitations in what you can determine from the given analysis
      5. Provide concrete recommendations where applicable, based on the technologies identified
...
function parseRepomixOutput(xmlContent: string, repoUrl: string, repoName: string): any {
  try {
    ...
    // Extract code samples for key files
    const codeFiles: Record<string, string> = {};
    const fileMatches = xmlContent.match(/<file\s+path="([^"]+)">\s*<content>\s*<!\[CDATA\[([\s\S]*?)\]\]>\s*<\/content>\s*<\/file>/g);
    
    if (fileMatches) {
      // Keep track of total content length to avoid making the API call too large
      let totalContentLength = 0;
      const MAX_CONTENT_LENGTH = 100000; // Limit total code samples to ~100K characters
      ...
      for (const match of fileMatches) {
        ...
        if (fileMatch) {
          ...
          // Only include important files (e.g. main files, config files, etc.)
          ...
          if (isImportantFile && totalContentLength + fileContent.length < MAX_CONTENT_LENGTH) {
            codeFiles[filePath] = fileContent;
            totalContentLength += fileContent.length;
          }
          
          // If we reach the max content length, stop adding files
